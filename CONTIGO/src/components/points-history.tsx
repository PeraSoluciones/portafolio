...
import { fetchPointsHistory, formatTransactionForDisplay, PointsHistoryResponse } from '@/lib/services/points-history-service';
...
export function PointsHistory({ childId, childName, currentBalance }: PointsHistoryProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Error al cargar el historial de puntos');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const moreTransactions = await fetchPointsHistory(childId, 20, data.transactions.length);
    ...
    return (
      <Card className="w-full max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>Cargando historial de puntos...</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-2xl font-bold text-success">{data.stats.total_earned}</p>
